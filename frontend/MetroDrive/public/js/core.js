...
app.controller('MainController', function($scope, $http) {
	...
	$scope.validate = function() {
		$http.get("/api/users?n=" + $scope.utente.username + "&p=" + $scope.utente.password)
		.then(function(response) {
			if (response.data.length == 1) {
				var singletonUser = utenteSingleton.createInstance($scope.utente.username);
				$scope.ooo = singletonUser.name;
				//location.href = "http://localhost:3000/veicoli?n=" + $scope.utente.username;
			} else {
